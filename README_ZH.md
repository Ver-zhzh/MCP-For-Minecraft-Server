...
#### 1. connect
使用API密钥作为标识符连接到Minecraft服务器。

**参数：**
- `url` - 服务器API URL（例如：http://localhost:8080）
- `api_key` - 用于认证的API密钥
- `timeout` - 请求超时时间（秒）（可选，默认：10）

**示例：**
```
AI："连接到我的Minecraft服务器localhost:8080"
响应："成功连接到Paper 1.20.4服务器"
```

#### 2. disconnect
通过API密钥断开与Minecraft服务器的连接。

**参数：**
- `api_key` - 要断开连接的服务器的API密钥
...
#### 3. list_servers
列出所有已连接的Minecraft服务器。

**示例：**
```
AI："显示所有已连接的服务器"
响应："您有2个已连接的服务器：服务器1（localhost:8080）、服务器2（192.168.1.100:8080）"
```
...
```
AI："服务器上有哪些可用命令？"
响应："找到156个命令，包括：give、teleport、gamemode、time、weather..."
```
